...
export default function AdminUsers() {
  ...
  const [passwordDialogUser, setPasswordDialogUser] = useState<{ id: string; name: string }>({ id: '', name: '' });
  ...
  const openPasswordDialog = (userId: string, userName: string) => {
    setPasswordDialogUser({ id: userId, name: userName });
          ...
          !isAdmin ? (
            ...
            <>
              <div className="flex items-center gap-3 p-4 rounded-lg border border-destructive/50 bg-destructive/10">
                ...
                <div>
                  <p className="font-semibold text-destructive text-sm">⚠️ PROTOTYPE ONLY — Chỉ dành cho demo nội bộ</p>
                  <p className="text-xs text-muted-foreground">Tính năng xem mật khẩu chỉ phục vụ mục đích demo. KHÔNG sử dụng trong môi trường thực tế.</p>
                </div>
              </div>
                        ...
                        <div className="flex items-center gap-2">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem onClick={() => openPasswordDialog(u.id, u.full_name)}>
