...
import { 
  ...
  ExternalLink, LogIn, LayoutDashboard, FileText, Menu, X, FolderKanban, FolderOpen, Trophy
} from 'lucide-react';
...
import PublicScoresView from '@/components/public/PublicScoresView';
...
type TabValue = 'overview' | 'tasks' | 'scores' | 'resources' | 'members' | 'activity';

export default function PublicProjectView() {
  const { shareToken, projectSlug } = useParams<{ shareToken?: string; projectSlug?: string }>();
  const lookupValue = projectSlug || shareToken;
  ...
  useEffect(() => {
    if (lookupValue) fetchPublicData();
  }, [lookupValue]);
  ...
  const fetchPublicData = async () => {
    try {
      ...
      const { data: groupData, error: groupError } = await supabase
        .from('groups')
        .select('*')
        .or(`share_token.eq.${lookupValue},slug.eq.${lookupValue}`)
        .eq('is_public', true)
        .single();
  ...
  const navItems = [
    ...
    ...(group?.show_tasks_public !== false ? [{ id: 'tasks' as TabValue, name: 'Task & Giai đoạn', icon: Layers }] : []),
    ...(group?.show_scores_public ? [{ id: 'scores' as TabValue, name: 'Điểm', icon: Trophy }] : []),
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 pt-16">
        <div className="max-w-[1600px] mx-auto p-6">
          ...
          {activeTab === 'tasks' && group.show_tasks_public !== false && (
            <PublicTaskListView 
              stages={stages} 
              tasks={tasks}
              groupId={group.id}
          ...
          {activeTab === 'scores' && group.show_scores_public && (
            <PublicScoresView groupId={group.id} members={members} stages={stages} />
          )}
