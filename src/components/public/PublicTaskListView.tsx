...
export default function PublicTaskListView({ stages, tasks, groupId }: PublicTaskListViewProps) {
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        return parsed.map(item => ({
          ...item,
          // Normalize: link items may have link_url or url
          url: item.url || item.link_url || item.submission_link || null,
  ...
  const handleOpenItem = async (item: any, taskId?: string, e?: React.MouseEvent) => {
    ...
    if (item.type === 'file' && item.file_path) {
      ...
      const siblings = fileItems.map((i: any) => ({
        file_path: i.file_path,
        file_name: i.file_name || 'file',
        file_size: i.file_size || 0,
        storage_name: 'task-submissions',
      }));
      const idx = fileItems.findIndex((i: any) => i.file_path === item.file_path);
