...
export default function SubmissionHistoryPopup({ 
    ...
    try {
      ...
      if (historyData && historyData.length > 0) {
        ...
        setHistory(historyData.map(h => ({
          ...h,
          user_name: profileMap.get(h.user_id)?.name || 'Unknown',
          user_avatar: profileMap.get(h.user_id)?.avatar || null,
        })));
      } else {
        setHistory([]);
      }
  ...
  const handleViewFile = async (filePath: string, fileName: string, fileSize: number, allFiles?: any[]) => {
    ...
    if (allFiles && allFiles.length > 0) {
      ...
      const clickedIndex = siblingFiles.findIndex((f: any) => f.file_path === filePath);
      openFilePreview(data.publicUrl, fileName, {
        siblingFiles,
                  ...
                  return (
                    <div 
                      ...
                      className={`p-5 rounded-2xl border-2 transition-all ${
                        isLatest 
                          ? 'border-foreground/20 bg-muted/50 shadow-lg shadow-foreground/5' 
                    ...
                    >
                      ...
                      <div className="flex items-start justify-between gap-4 mb-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {isLatest && (
                            <Badge className="text-xs px-3 py-1 bg-foreground text-background">
                              Mới nhất
                            </Badge>
                          )}
                          {isLate && (
                          ...
                          if (link.type === 'file' || link.file_path) {
                            return (
                              <button
                                ...
                                className="flex items-center gap-3 p-3 rounded-xl bg-background border-2 border-transparent hover:border-border hover:bg-muted/50 transition-all group text-left"
                              >
                                <div className="p-2 rounded-lg bg-muted group-hover:bg-muted">
                            ...
                            return (
                              <a
                                ...
                                className="flex items-center gap-3 p-3 rounded-xl bg-background border-2 border-transparent hover:border-border hover:bg-muted/50 transition-all group"
                              >
                                <div className="p-2 rounded-lg bg-muted group-hover:bg-muted">
