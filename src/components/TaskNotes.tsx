    ...
    deleteWithUndo({
      ...
      onDelete: async () => {
        for (const attachment of deletedAttachments) {
          await r2Storage.from('task-note-attachments').remove([attachment.file_path]);
        }
                          ...
                          >
                            ...
                            <div className="flex items-center gap-1 shrink-0">
                              <Button 
                                ...
                                className="h-7 w-7"
                                onClick={() => {
                                  const { data } = r2Storage.from('task-note-attachments').getPublicUrl(attachment.file_path);
                                  const siblings = attachments.map((a: NoteAttachment) => ({
                                    file_path: a.file_path,
                                    file_name: a.file_name,
                                    file_size: a.file_size,
                                    storage_name: 'task-note-attachments',
                                  }));
                                  const idx = attachments.findIndex((a: NoteAttachment) => a.id === attachment.id);
                                  openFilePreview(data.publicUrl, attachment.file_name, {
