...
export function AdminBillingUsersTab() {
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <Input placeholder={t?.searchPlaceholder || 'Search...'} value={search} onChange={e => setSearch(e.target.value)} className="pl-9" />
        </div>
        ...
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger className="w-[150px]"><SelectValue placeholder={t?.filterByStatus || 'All Statuses'} /></SelectTrigger>
          <SelectContent>
            <SelectItem value="all">{t?.filterByStatus || 'All Statuses'}</SelectItem>
            {['active', 'expired', 'trial', 'suspended', 'cancelled', 'grace_period'].map(s => <SelectItem key={s} value={s}>{statusLabel(s)}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
          ...
          <TableBody>
            {filtered.map((user: any) => (
              <TableRow key={user.id} className="cursor-pointer" onClick={() => navigate(`/admin/billing/${user.id}`)}>
                ...
                <TableCell className="text-sm text-muted-foreground">{user.plan_expires_at ? format(new Date(user.plan_expires_at), 'dd/MM/yyyy') : '—'}</TableCell>
                <TableCell>
                  <Button size="sm" variant="ghost" onClick={e => { e.stopPropagation(); navigate(`/admin/billing/${user.id}`); }}>
