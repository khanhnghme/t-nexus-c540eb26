...
export default function MultiFileUploadSubmission({
  ...
  const handleFilesSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        // Use R2 storage upload
        const { data, error } = await r2Storage.from('task-submissions')
      ...
      for (const uploadedFile of newUploadedFiles) {
        try {
          await r2Storage.from('task-submissions').remove([uploadedFile.file_path]);
        } catch (e) {
  ...
  const handleRemoveFile = async (fileToRemove: UploadedFile) => {
    try {
      await r2Storage.from('task-submissions').remove([fileToRemove.file_path]);
